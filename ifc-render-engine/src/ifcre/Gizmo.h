...
namespace ifcre {
	...
	struct SceneGizmo {
		...
		glm::mat4 private_transform(const glm::vec2 window_size) {
			...
			//glm::vec3 window_scale = glm::vec3((window_size.y - 65) / window_size.x, 1.f, 1.f) * (20 / window_size.y); // .2可以设置gizmo大小
			glm::vec3 window_scale = glm::vec3((window_size.y - 65) / window_size.x, 1.f, 1.f) * (260 / window_size.y); // .2可以设置gizmo大小
