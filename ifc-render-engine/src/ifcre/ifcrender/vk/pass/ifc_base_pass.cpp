...
namespace ifcre {
    ...
    void VulkanIFCBasePass::draw(uint32_t mesh_id, bool bbx_draw)
    {
...
#ifndef _DEBUG
        VulkanBuffer& edge_index_bufer = *(mesh_buffer.edgeIndexBuffer);
        if (edge_index_bufer.getSize() > 0) {
            ctx.fp_vkCmdBindVertexBuffers(cmd_buffer, 0, 1, vertex_buffers, offsets);
            ctx.fp_vkCmdBindPipeline(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_renderPipeline[render_pipeline_edge].pipeline);
            ctx.fp_vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_renderPipeline[render_pipeline_edge].layout, 0, 1, &m_descriptorInfos[layout_edge].descriptor_set, 0, nullptr);
            ctx.fp_vkCmdBindIndexBuffer(cmd_buffer, edge_index_bufer.getBuffer(), 0, VK_INDEX_TYPE_UINT32);
            ctx.fp_vkCmdDrawIndexed(cmd_buffer, edge_index_bufer.getSize(), 1, 0, 0, 0);
        }
#endif

        // render bounding box
        if (bbx_draw) {
            VulkanBuffer& bounding_box_vertex_buffer = *(m_vulkanResources->boundingBoxMeshBuffer.vertexBuffer);
            VulkanBuffer& bounding_box_index_buffer = *(m_vulkanResources->boundingBoxMeshBuffer.IndexBuffer);
            VkBuffer bounding_box_vertex_buffers[] = { bounding_box_vertex_buffer.getBuffer() };
            ctx.fp_vkCmdBindPipeline(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_renderPipeline[render_pipeline_bounding_box].pipeline);
            ctx.fp_vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_renderPipeline[render_pipeline_bounding_box].layout, 0, 1, &m_descriptorInfos[layout_bounding_box].descriptor_set, 0, nullptr);
            ctx.fp_vkCmdBindVertexBuffers(cmd_buffer, 0, 1, bounding_box_vertex_buffers, offsets);
            ctx.fp_vkCmdBindIndexBuffer(cmd_buffer, bounding_box_index_buffer.getBuffer(), 0, VK_INDEX_TYPE_UINT32);
            ctx.fp_vkCmdDrawIndexed(cmd_buffer, bounding_box_index_buffer.getSize(), 1, 0, 0, 0);
        }
    ...
    void VulkanIFCBasePass::_createDescriptorSetLayouts()
    {
...

        {
            ...
            VK_CHECK_RESULT(vkCreateDescriptorSetLayout(ctx.m_device, &layout_info, nullptr, &m_descriptorInfos[layout_edge].layout));
        }

        {
            ...
            VK_CHECK_RESULT(vkCreateDescriptorSetLayout(ctx.m_device, &layout_info, nullptr, &m_descriptorInfos[layout_bounding_box].layout));
        }
    ...
    void VulkanIFCBasePass::_createDescriptorSets()
    {
        ...
        // edge alloc
        {
            ...
            alloc_info.descriptorPool = m_descriptorPool;
            alloc_info.descriptorSetCount = 1U;
            alloc_info.pSetLayouts = &m_descriptorInfos[layout_edge].layout;

            VK_CHECK_RESULT(vkAllocateDescriptorSets(ctx.m_device, &alloc_info, &m_descriptorInfos[layout_edge].descriptor_set));

            auto& uniform_buffer_map = m_vulkanResources->uniformBufferMap;

            VkDescriptorBufferInfo transforms_buffer_info{};
            transforms_buffer_info.buffer = uniform_buffer_map[uniform_buffer_transform_mvp]->getBuffer();
            transforms_buffer_info.offset = 0;
            transforms_buffer_info.range = sizeof(TransformMVPUBO);

            std::array<VkWriteDescriptorSet, 1> write_desc_sets{};
            write_desc_sets[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            write_desc_sets[0].dstSet = m_descriptorInfos[layout_edge].descriptor_set;
            write_desc_sets[0].dstBinding = 0;
            write_desc_sets[0].dstArrayElement = 0;
            write_desc_sets[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            write_desc_sets[0].descriptorCount = 1;
        ...
        // bounding box alloc
        {
            ...
            alloc_info.descriptorPool = m_descriptorPool;
            alloc_info.descriptorSetCount = 1U;
            alloc_info.pSetLayouts = &m_descriptorInfos[layout_bounding_box].layout;

            VK_CHECK_RESULT(vkAllocateDescriptorSets(ctx.m_device, &alloc_info, &m_descriptorInfos[layout_bounding_box].descriptor_set));
            ...
            std::array<VkWriteDescriptorSet, 1> write_desc_sets{};
            write_desc_sets[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            write_desc_sets[0].dstSet = m_descriptorInfos[layout_bounding_box].descriptor_set;
            write_desc_sets[0].dstBinding = 0;
            write_desc_sets[0].dstArrayElement = 0;
            write_desc_sets[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            write_desc_sets[0].descriptorCount = 1;
    ...
    void VulkanIFCBasePass::_createPipeline()
    {
        ...
        {
#ifdef _DEBUG
            auto vert_code = VulkanUtil::compileFile("shaders/edge.vert", shaderc_glsl_vertex_shader);
            auto frag_code = VulkanUtil::compileFile("shaders/edge.frag", shaderc_glsl_fragment_shader);
#else
            auto vert_code = VulkanUtil::compileString(new_sc::v_edge, shaderc_glsl_vertex_shader, true);
            auto frag_code = VulkanUtil::compileString(new_sc::f_edge, shaderc_glsl_fragment_shader, true);
#endif
            VkShaderModule vert_shader_module = VulkanUtil::createShaderModule(ctx.m_device, vert_code);
            VkShaderModule frag_shader_module = VulkanUtil::createShaderModule(ctx.m_device, frag_code);
            ...
            pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
            pipelineLayoutInfo.setLayoutCount = 1;
            pipelineLayoutInfo.pSetLayouts = &m_descriptorInfos[layout_edge].layout;
            pipelineLayoutInfo.pushConstantRangeCount = 0;

            VK_CHECK_RESULT(vkCreatePipelineLayout(ctx.m_device, &pipelineLayoutInfo, nullptr, &m_renderPipeline[render_pipeline_edge].layout));
            ...
            pipelineInfo.pMultisampleState = &multisampling;
            pipelineInfo.pColorBlendState = &colorBlending;
            pipelineInfo.layout = m_renderPipeline[render_pipeline_edge].layout;
            pipelineInfo.renderPass = m_framebuffer.render_pass;
            pipelineInfo.subpass = 0;
            pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;
            pipelineInfo.pDepthStencilState = &depthStencil;
            ...
            VK_CHECK_RESULT(vkCreateGraphicsPipelines(ctx.m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_renderPipeline[render_pipeline_edge].pipeline));
		...
		{
#ifdef _DEBUG
		 auto vert_code = VulkanUtil::compileFile("shaders/bbx.vert", shaderc_glsl_vertex_shader);
		 auto frag_code = VulkanUtil::compileFile("shaders/bbx.frag", shaderc_glsl_fragment_shader);
#else
		 auto vert_code = VulkanUtil::compileString(new_sc::v_bbx, shaderc_glsl_vertex_shader, true);
		 auto frag_code = VulkanUtil::compileString(new_sc::f_bbx, shaderc_glsl_fragment_shader, true);
#endif
		 VkShaderModule vert_shader_module = VulkanUtil::createShaderModule(ctx.m_device, vert_code);
		 VkShaderModule frag_shader_module = VulkanUtil::createShaderModule(ctx.m_device, frag_code);
		 ...
		 pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
		 pipelineLayoutInfo.setLayoutCount = 1;
		 pipelineLayoutInfo.pSetLayouts = &m_descriptorInfos[layout_bounding_box].layout;
		 pipelineLayoutInfo.pushConstantRangeCount = 0;

		 VK_CHECK_RESULT(vkCreatePipelineLayout(ctx.m_device, &pipelineLayoutInfo, nullptr, &m_renderPipeline[render_pipeline_bounding_box].layout));
		 ...
		 pipelineInfo.pMultisampleState = &multisampling;
		 pipelineInfo.pColorBlendState = &colorBlending;
		 pipelineInfo.layout = m_renderPipeline[render_pipeline_bounding_box].layout;
		 pipelineInfo.renderPass = m_framebuffer.render_pass;
		 pipelineInfo.subpass = 0;
		 pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;
		 pipelineInfo.pDepthStencilState = &depthStencil;
		 ...
		 VK_CHECK_RESULT(vkCreateGraphicsPipelines(ctx.m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_renderPipeline[render_pipeline_bounding_box].pipeline));
